...
class Stream(Iterable[T]):
    ...
    def __init__(self, data: "Iterable[T] | Iterator[T] | EllipsisType") -> None:
        """Create a new Stream.
    ...
    def map(self, fun: Callable[[T], K]) -> "Stream[K]":
        ...
        """
        self._check_finished()
        return self._finish(Stream(map(fun, self._data)))  # pylint: disable=bad-builtin
    ...
    @overload
    def collect(self: "Stream[tuple[K, V]]", fun: type[dict[Any, Any]]) -> dict[K, V]:
        ...

    @overload
    def collect(
        self,
        fun: Callable[[Iterator[T]], K]
    ) -> K:
        ...
